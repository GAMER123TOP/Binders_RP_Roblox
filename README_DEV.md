...
> **Почему у Провинции префикс `Province_`?**
> 
> В Провинции используется отдельная клавиша для открытия чата (`U` / `Г`), поэтому все скрипты для неё имеют отдельные названия, чтобы не путаться с другими играми.
...
2. Вместо `.txt` напиши `.ahk`:
   - `MVD_Macros.ahk` (или `[Игра]_MVD_Macros.ahk`, если нужен префикс)
   - И другие файлы
